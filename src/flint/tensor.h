...
class Tensor {
 public:
  ...
  template<typename T>
  static Tensor create(ly::Span<const int> shape, ly::Span<const T> data);
  ...
  const TensorData *getDataObject() const { return _data.get(); }
 ...
 protected:
  ...
  int8_t*getRawData(DType dtype) const;
};
