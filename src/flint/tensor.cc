...
void Tensor::read(ly::ReadableFile *fp) {
  ...
  for (int16_t d = 0; d < rank; ++d) {
    ...
    if (dimension > 65536) {
      throw ly::AbortedError("dimension too big");
    }
    numel *= dimension;
  ...
  _shape = TensorShape(ly::makeConstSpan(shape));

  // data
  _data = TensorData::create(numel, dtype);
  ly::Span<int8_t> bs_data(_data->getData(), numel);
  fp->readSpan(ly::makeSpan(_data->getData(), _data->getSizeInBytes()));
  _dataPtr = _data->getData();

  // magic number
  int16_t magic_number = fp->readValue<int16_t>();
  if (magic_number != 0x55aa) {
    throw ly::AbortedError("invalid magic number");
  }
}

int8_t *Tensor::getRawData(DType dtype) const {
  CHECK(_data);
...
Tensor Tensor::view(ly::Span<const int> shape) const {
  ...
  view._shape = TensorShape(ly::makeConstSpan(realShape));
