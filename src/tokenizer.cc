...
class BPEModel : public Vocab, private util::NonCopyable {
 public:
  // token flags.
...
void BPEModel::readModel(ReadableFile *fp) {
  ...
  if (s != "LLsp") {
    throw AbortedException("bad format (header)");
  }
...
void BPEModel::readMagicNumber(ReadableFile *fp) {
  ...
  if (magic_number != kMagicNumber) {
    throw AbortedException("bad format (magic number)");
  }
}
...
void BPEModel::initModel() {
  ...
  for (const TokenInfo &info : _tokens) {
    if (!info.flag) {
      ...
      if (_unkId != kInvalidToken) {
        throw AbortedException("bad format (too many unknown tokens)");
      }
      _unkId = info.id;
    }
  }
  ...
  if (itSpace == _tokenDict.end()) {
    throw AbortedException("bad format (no symbol for space)");
  }
...
BPEModel::TokenInfo BPEModel::readRecord(ReadableFile *fp) {
  ...
  if ((info.flag & kByte) && info.tokenPiece.size() != 1) {
    throw AbortedException("bad format (byte)");
  }
...
void BPEModel::checkModel() {
  for (int ch = 0; ch < 256 && _isByteTokenAvailable; ++ch) {
    if (_byteId[ch] == kInvalidToken)
      throw AbortedException(str::sprintf("bad format, byte %d not exist in model", ch));
  }

  if (_unkId == kInvalidToken) {
    throw AbortedException("bad model (no unknown token)");
  }
}
...
class BPEEncoder : private util::NonCopyable {
 ...
 private:
  ...
  util::Pool<Symbol, kSymbolPoolBlockSize> _symbolPool;
  Symbol *_header;
...
void BPEEncoder::initSymbolList(const std::string &s) {
  ...
  if (_config->splitByUnicode) {
    initialPieces = str::splitToUtf8Chars(s);
  } else {
...
std::unique_ptr<Tokenizer> Tokenizer::create(const IniSection &config) {
  ...
  if (type == "bpe") {
    ...
    throw AbortedException(str::sprintf("invalid tokenizer type: %s", type));
  }
}
